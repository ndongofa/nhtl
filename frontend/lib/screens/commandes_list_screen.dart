...
// Couleurs/icônes alignées sur le backend (sans accents)
final Map<String, Color> statutColors = {
  "EN_ATTENTE": Colors.grey,
  "CONFIRMEE": Colors.green,
  "EN_TRAITEMENT": Colors.orange,
  "EXPEDIEE": Colors.blueGrey,
  "LIVREE": Colors.blue,
  "ANNULEE": Colors.red,
...
class _CommandesListScreenState extends State<CommandesListScreen> {
  ...
  Future<void> _editCommande(Commande commande) async {
    final refresh = await Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => CommandeFormScreen(commande: commande)),
    );
    if (refresh == true) {
      _refresh();
    }
  ...
  Future<void> _assignGpToCommande(Commande commande) async {
    ...
    try {
      await _gpService.assignGpToCommande(
        ...
        gpId: gp.id,
        newStatut: "EN_TRAITEMENT", // harmonisé backend
      );
              ...
              itemBuilder: (context, index) {
                ...
                return CommandeTile(
                  ...
                  onStatutChanged: logged.role == "admin"
                      ? (newStatut) =>
                          _changeStatutCommande(commande.id!, newStatut)
                      : null,
